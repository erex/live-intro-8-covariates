...
date: Other model selection metrics <br> September 2020
output:
...
The penalty term changes as a function of sample size; the larger the number of detections, the smaller the effect of the detection function.
...
Fit a series of models to the amakihi data set, described in Marques et al. (2007).  Call the `altmetric()` function for each of the fitted models, combine the resulting vectors into a data frame.

```{r fits, message=FALSE}
conv <- convert_units("meter", NULL, "hectare")
amak.hn <- ds(amakihi, transect="point", key="hn", convert.units = conv, truncation=82.5)
amak.hr <- ds(amakihi, transect="point", key="hr", convert.units = conv, truncation=82.5)
amak.hr.obs <- ds(amakihi, transect="point", key="hr", formula=~OBs, convert.units = conv, 
                  truncation=82.5)
amak.hr.mas <- ds(amakihi, transect="point", key="hr", formula=~MAS, convert.units = conv, 
                  truncation=82.5)
